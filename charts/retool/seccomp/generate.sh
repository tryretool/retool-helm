...
# that follow).
insert_rules() {
  local input="$1" additions="$2" dest="$3"
  jq --slurpfile add "$additions" '
    ($add[0]) as $new_rules |
    (.syscalls | to_entries
...
JSON
  ...
  insert_rules "$CACHE_FILE" "$additions" "$dest"
  rm -f "$additions"
}
...
JSON
  ...
  patched="$(mktemp)"
  jq '
    .comment = "Docker default seccomp profile + gVisor/pasta additions. defaultErrnoRet changed to ENOSYS (38) so gVisor can distinguish blocked syscalls from unimplemented ones. archMap narrowed to x86_64 + aarch64 (gVisor supported platforms)." |
    .defaultErrnoRet = 38 |
    .archMap = [
  ...
  insert_rules "$patched" "$additions" "$dest"
